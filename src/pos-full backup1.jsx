...
const DEFAULT_PRODUCTS = [
  { id: "p1", name: "Lao Coffee", sku: "DRK-001", cost: 8000, price: 15000, category: "Drinks", stock: 100, lowStockThreshold: 10, unit: "cup", barcode: "8850001" },
  { id: "p2", name: "Green Tea", sku: "DRK-002", cost: 5000, price: 10000, category: "Drinks", stock: 80, lowStockThreshold: 10, unit: "cup", barcode: "8850002" },
  { id: "p3", name: "Baguette Sandwich", sku: "FOD-001", cost: 12000, price: 25000, category: "Food", stock: 30, lowStockThreshold: 5, unit: "pc", barcode: "8850003" },
  { id: "p4", name: "Khao Piak Sen", sku: "FOD-002", cost: 15000, price: 30000, category: "Food", stock: 25, lowStockThreshold: 5, unit: "bowl", barcode: "8850004" },
  { id: "p5", name: "Sticky Rice", sku: "FOD-003", cost: 3000, price: 5000, category: "Food", stock: 50, lowStockThreshold: 10, unit: "portion", barcode: "8850005" },
  { id: "p6", name: "Fresh Juice", sku: "DRK-003", cost: 7000, price: 15000, category: "Drinks", stock: 40, lowStockThreshold: 8, unit: "glass", barcode: "8850006" },
  { id: "p7", name: "Water Bottle", sku: "DRK-004", cost: 2000, price: 5000, category: "Drinks", stock: 200, lowStockThreshold: 20, unit: "bottle", barcode: "8850007" },
  { id: "p8", name: "Lao Chips", sku: "SNK-001", cost: 4000, price: 8000, category: "Snacks", stock: 60, lowStockThreshold: 10, unit: "bag", barcode: "8850008" },
];
...
export default function FullPOS() {
  ...
  const exportToExcel = () => {
    ...
    let csv = "\uFEFF";
    csv += `${storeSettings.name} - SALES REPORT\n`;
    csv += `Period:,${dateFilter === "all" ? "All Time" : dateFilter}\n`;
    csv += `Generated:,${fmtFull(new Date())}\n\n`;
    csv += "SUMMARY\n";
    csv += `Gross Revenue:,${totals.revenue}\nTotal Cost:,${totals.cost}\nGross Profit:,${totals.profit}\n`;
    csv += `Discounts Given:,${totals.discounts}\nTax Collected:,${totals.tax}\nRefunds:,${totals.refunds}\n`;
    csv += `Net Revenue:,${netRevenue}\nTransactions:,${totals.transactions}\nProfit Margin:,${pct(totals.profit, totals.revenue)}\n\n`;
    csv += "DETAILED TRANSACTIONS\n";
    csv += "Receipt#,Date,Time,Customer,Item,SKU,Qty,Unit Cost,Unit Price,Discount,Line Total,Line Cost,Line Profit,Payment,Status\n";
    filtered.forEach((sale) => sale.items.forEach((item) => {
      csv += `${sale.receiptNo},${fmtDate(sale.date)},${fmtTime(sale.date)},${sale.customerName},${item.name},${item.sku || ""},${item.qty},${item.cost},${item.price},${item.itemDiscount || 0},${item.lineTotal},${item.lineCost},${item.lineProfit},${sale.paymentMethod},${sale.status}\n`;
    }));
    csv += `\n,,,,,,,,,TOTALS:,${totals.revenue},${totals.cost},${totals.profit}\n\n`;
    ...
    Object.entries(perfMap).sort((a, b) => b[1].rev - a[1].rev).forEach(([n, d]) => {
      csv += `${n},${d.sku},${d.qty},${d.rev},${d.cost},${d.profit},${pct(d.profit, d.rev)}\n`;
    });
    csv += "\nINVENTORY STATUS\nProduct,SKU,Current Stock,Unit,Low Stock Alert\n";
    products.forEach((p) => { csv += `${p.name},${p.sku},${p.stock},${p.unit},${p.stock <= p.lowStockThreshold ? "LOW" : "OK"}\n`; });
    csv += "\nCUSTOMER SUMMARY\nName,Phone,Visits,Total Spent,Loyalty Points\n";
    customers.forEach((c) => { csv += `${c.name},${c.phone || ""},${c.visits || 0},${c.totalSpent || 0},${c.loyaltyPoints || 0}\n`; });
    csv += "\nPAYMENT METHODS\nMethod,Count,Amount\n";
    const pmMap = {};
    ...
    Object.entries(pmMap).forEach(([m, d]) => { csv += `${m},${d.count},${d.amt}\n`; });

    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = `${storeSettings.name.replace(/\s/g, "_")}_Report_${new Date().toISOString().slice(0, 10)}.csv`;
                ...
                {filteredProducts.map((p) => {
                  ...
                  return (
                    <button key={p.id} onClick={() => addToCart(p)} disabled={outOfStock}
                      ...
                      <div style={{ fontFamily: S.mono, fontSize: "14px", fontWeight: 700, color: cc.text, marginTop: "4px" }}>{fmt(p.price)}</div>
          ...
          <div style={{ padding: "20px 24px", maxWidth: "1100px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button onClick={() => {
                  setEditProduct({ id: null, name: "", sku: "", cost: "", price: "", category: "Other", stock: "", lowStockThreshold: 10, unit: "pc", barcode: "" });
              ...
              <div style={{ ...S.card, padding: "20px", marginBottom: "16px", border: "2px solid #111827" }} className="fade-in">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "12px" }}>
                  {[
                    ...
                    { key: "price", label: "Sell Price (₭)", type: "number", ph: "0" },
                ...
                <div style={{ display: "flex", gap: "8px", marginTop: "16px" }}>
                  <button onClick={() => {
                    ...
                    const p = { ...editProduct, cost: Number(editProduct.cost), price: Number(editProduct.price), stock: Number(editProduct.stock) || 0, lowStockThreshold: Number(editProduct.lowStockThreshold) || 10 };
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid #e5e7eb" }}>
                    {["Product", "SKU", "Category", "Cost", "Price", "Margin", "Stock", "Actions"].map((h) => (
                      ...
                      <tr key={p.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                        ...
                        <td style={{ padding: "10px 12px" }}>
                          <div style={{ display: "flex", gap: "4px" }}>
                            <button onClick={() => setEditProduct({ ...p, cost: String(p.cost), price: String(p.price), stock: String(p.stock), lowStockThreshold: String(p.lowStockThreshold) })}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "20px" }}>
              ...
              <div style={{ ...S.card, padding: "20px" }}>
                ...
                {(() => {
                  ...
                  const colors = { cash: "#059669", card: "#2563eb", transfer: "#7c3aed", qr: "#f59e0b" };
